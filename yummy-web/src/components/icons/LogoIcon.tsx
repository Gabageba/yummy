...
function InternalIcon(props: CustomIconComponentProps | React.SVGProps<SVGSVGElement>) {
  ...
  return (
    <svg
      ...
      {...rest}
    >
      <rect width="64" height="64" rx="16" fill="#F7A348" />
      <path
...
function LogoIcon({ size, style, ...restProps }: IProps) {
  const resolvedStyle = size !== undefined ? { fontSize: size, ...style } : style;
  return <Icon component={InternalIcon} style={resolvedStyle} {...restProps} />;
}
